<template lang="html">
  <div class="table-repsonsive">
    <table v-if="invoiceExist === true" class="table table-bordered">
      ...
      <tbody>
        ...
        >
          ...
          <td>{{ item.attributes.date }}</td>
          <td>${{ item.attributes.amount }}</td>
          <td v-if="item.attributes.paid === true" class="status-color">
            {{ item.attributes.status }}
          </td>
          <td v-else class="status-color--pending">
            {{ item.attributes.status }}
          </td>
        </tr>
      </tbody>
    ...
    <div class="ps-pagination" v-if="invoiceExist">
      <ul class="pagination">
  ...
  methods: {
        ...
        .then((ress) => {
          ...
          if (ress.data.length > 0) {
            ...
            for (let i = 0; i < res.length; i++) {
              ...
              res[i].attributes.date = new Date(
                res[i].attributes.createdAt
              ).toLocaleDateString();
        ...
        .then((ress) => {
          ...
          if (ress.data.data.length > 0) {
            ...
            if (this.page === 1) {
              ...
              for (let i = 0; i < res.length; i++) {
                res[i].attributes.id_invoice_user =
                  (this.page - 1) * 10 + (i + 1);
                res[i].attributes.date = new Date(
                  res[i].attributes.createdAt
                ).toLocaleDateString();
    ...
    async changePage() {
      ...
      const respuesta = await this.$store
        ...
        .then((ress) => {
          if (
            ress.data.data.length > 0 &&
            ress.data.meta.pagination.pageCount >= this.page
          ) {
