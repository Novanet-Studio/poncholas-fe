		...
		<aside class="widget widget_shop">
			...
			<ul v-if="categories !== undefined" class="ps-list--categories">
				...
				<li v-for="category in categories" :key="category.id">
					<a href="#" @click.prevent="handleGotoCategory(category.slug)">
						{{ category.name }}
					</a>
				</li>
			</ul>
	...
	methods: {
		...
		async handleFilterByBrand() {
			if (this.selectedBrands) {
				await this.$store.commit("collection/setQueries", this.selectedBrands);
		...
		async handleFilterByPriceRagne() {
			...
			await this.$store.dispatch("product/getProductsByPriceRange", params);
